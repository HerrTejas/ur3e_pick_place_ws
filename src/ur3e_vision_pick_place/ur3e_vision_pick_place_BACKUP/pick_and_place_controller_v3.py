#!/usr/bin/env python3
"""
Pick and Place Controller v3 — Clean State Machine

Long moves: joint-space (one IK solve, smooth trapezoidal interpolation)
Short vertical moves: Cartesian (stable IK chain on Z-only paths)

Why v3:
- v1 (all-Cartesian for long moves) suffered IK branch drift mid-path.
- v2 added joint-space for long moves but kept PRE_GRASP_ALIGN and PRE_PLACE
  as Cartesian re-alignment steps. Each one re-publishes a near-zero-distance
  trajectory and re-solves IK ~40 times for sub-cm motion → controller jerks.
- v3 trusts joint-space IK orientation (1e-4 rad tolerance ≈ 0.006°) and
  drops the alignment steps. Vertical Cartesian segments that follow only
  SLERP across ~0.0001 rad of orientation drift — invisible.
- Smart IK seed (pregrasp_seed) keeps solver in the natural elbow-up branch
  regardless of object position.

States:
  IDLE → DETECTING →
  PRE_GRASP        (joint-space) → HOLD →
  LOWER            (Cartesian, vertical) → CLOSE_GRIP → HOLD →
  LIFT             (Cartesian, vertical) → HOLD →
  MOVE_TO_PLACE    (joint-space) → HOLD →
  LOWER_PLACE      (Cartesian, vertical) → OPEN_GRIP → HOLD →
  RETREAT          (Cartesian, vertical) → HOLD →
  MID_SAFE         (joint-space) → HOLD →
  HOME             (joint-space) → IDLE
...
class PickAndPlaceController(Node):
    def __init__(self):
        super().__init__('pick_and_place_v3')
        ...
        try:
            ...
            self.get_logger().info('Pinocchio loaded')
        ...
        # Known-good config in the working area — used as IK seed reference.
        # Matches the proven RED_BOX_GRASP from trapezoidal_planner.
        self.WORKING_AREA_SEED = np.array(
            [1.255, -0.98, 1.4, -1.8, -1.61, -0.3]
        )
        ...
        self.get_logger().info('Pick and Place Controller v3 Ready!')
    ...
    def candidate_seeds(self):
        """
        Seeds to try in priority order.
    
        1. WORKING_AREA_SEED — known-good elbow-up branch, proven for this workspace.
        2. current joints — what worked in v2 (IK from HOME → cost ~4.8 but converged).
        3. HOME — last resort.
        """
        seeds = [self.WORKING_AREA_SEED.copy()]
        if self.current_positions is not None:
            seeds.append(np.array(self.current_positions[:6]))
        seeds.append(np.array(self.HOME))
        return seeds
    
    def solve_ik(self, x, y, z, max_cost=None, seed=None):
        """Solve IK with a list of seed candidates. Returns first one that fits."""
        target_pos = np.array([x, y, z])
    
        if seed is not None:
            seeds_to_try = [np.asarray(seed)]
        else:
            seeds_to_try = self.candidate_seeds()
    
        for idx, s in enumerate(seeds_to_try):
            q_sol = compute_ik(
                self.pin_model, self.pin_data, self.pin_ee_id,
                target_pos, self.grasp_rot, s)
    
            if q_sol is None:
                self.get_logger().warn(
                    f'  IK seed {idx} did not converge, trying next...')
                continue
            
            cost = self.joint_cost(self.current_positions[:6], q_sol)
            self.get_logger().info(
                f'  IK seed {idx} converged, cost vs current: {cost:.2f} rad')
    
            if max_cost is not None and cost > max_cost:
                self.get_logger().warn(
                    f'  Cost too high ({cost:.2f} > {max_cost:.1f}), '
                    f'trying next seed...')
                continue
        ...
        self.get_logger().error(
            f'  IK failed for ({x:.3f}, {y:.3f}, {z:.3f}) — all seeds exhausted')
        return None
    ...
    def joint_state_cb(self, msg):
        ...
        if len(velocities) == 6:
            self.current_velocities = [
                velocities[name] for name in self.joint_names]
    ...
    def execute_current_state(self):
        ...
        if self.state == 'PRE_GRASP':
            # JOINT-SPACE: long move from HOME to above object.
            # Smart seed (pregrasp_seed) keeps IK in the natural branch.
            z = obj[2] + z_off + self.pre_grasp_height
            q = self.solve_ik(obj[0], obj[1], z,
                              max_cost=self.ik_cost_threshold)
            if q is None:
                self.transition_to('IDLE')
                return
            self.get_logger().info(
                f'  Joint-space pre-grasp: '
                f'({obj[0]:.3f}, {obj[1]:.3f}, {z:.3f})')
            self.move_joints(q)

        elif self.state == 'LOWER':
            # CARTESIAN: straight down. IK chain is stable on Z-only paths.
            self.move_cartesian(
                obj[0], obj[1],
            ...
            # JOINT-SPACE: long lateral move to above place position.
            px, py = self.place_positions[self.active_color]
            place_z = obj[2] + z_off + self.lift_height
            q = self.solve_ik(px, py, place_z,
                              max_cost=self.ik_cost_threshold)
            if q is None:
                self.transition_to('IDLE')
                return
            self.get_logger().info(
                f'  Joint-space to place: '
                f'({px:.3f}, {py:.3f}, {place_z:.3f})')
            self.move_joints(q)

        elif self.state == 'LOWER_PLACE':
            # CARTESIAN: straight down to drop
            px, py = self.place_positions[self.active_color]
            self.move_cartesian(
                px, py,
    ...
    def check_arrival(self):
        # Adaptive HOLD: settle until velocity < tol for N cycles
        if self.state == 'HOLD':
        ...
        # Velocity check — must be slow when "arrived"
        if arrived and self.current_velocities is not None:
            max_vel = max(abs(v) for v in self.current_velocities)
            if max_vel > self.velocity_tolerance:
                arrived = False

        # Require N consecutive cycles
        if arrived:
            self.arrival_count += 1
            if self.arrival_count >= self.arrival_cycles_required:
                self.arrival_count = 0
    ...
    def next_state(self):
        # No PRE_GRASP_ALIGN, no PRE_PLACE — joint-space IK lands
        # within tolerance, vertical Cartesian doesn't need re-aligning.
        transitions = {
