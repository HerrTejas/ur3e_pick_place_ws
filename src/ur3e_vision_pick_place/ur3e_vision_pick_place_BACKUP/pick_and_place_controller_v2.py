...
Long moves: joint-space (no IK chain, no jumps)
Short moves: Cartesian (straight line, arm barely reconfigures)

States:
  IDLE → PRE_GRASP (joint) → HOLD → PRE_GRASP_ALIGN (Cartesian) →
  LOWER (Cartesian) → CLOSE_GRIP → HOLD → LIFT (Cartesian) →
  HOLD → MOVE_TO_PLACE (joint) → HOLD → PRE_PLACE (Cartesian) →
  LOWER_PLACE (Cartesian) → OPEN_GRIP → HOLD → RETREAT (Cartesian) →
  HOLD → MID_SAFE (joint) → HOLD → HOME (joint) → IDLE
...
class PickAndPlaceController(Node):
    def __init__(self):
        super().__init__('pick_and_place_v2')
    ...
    def solve_ik(self, x, y, z, max_cost=None):
        """Solve IK for a position with grasp orientation."""
        ...
        if max_cost is not None and cost > max_cost:
            self.get_logger().error(f'  IK cost too high ({cost:.2f} > {max_cost:.1f})!')
            return None
        return q_sol.tolist()
    ...
    def execute_current_state(self):
        ...
        if self.state == 'PRE_GRASP':
            ...
            if q is None:
                ...
                return
            self.get_logger().info(
                f'  Joint-space pre-grasp: ({obj[0]:.3f}, {obj[1]:.3f}, {z:.3f})')
            self.move_joints(q)

        elif self.state == 'PRE_GRASP_ALIGN':
            # CARTESIAN: tiny re-alignment after joint-space move
            self.move_cartesian(
                obj[0], obj[1],
                obj[2] + z_off + self.pre_grasp_height)

        elif self.state == 'LOWER':
            # CARTESIAN: straight down
            self.move_cartesian(
                obj[0], obj[1],
            ...
            q = self.solve_ik(px, py, place_z, max_cost=5.0)  # limit — should be ~3 rad
            if q is None:
                self.transition_to('IDLE')
                return
            self.get_logger().info(
                f'  Joint-space to place: ({px:.3f}, {py:.3f}, {place_z:.3f})')
            self.move_joints(q)

        elif self.state == 'PRE_PLACE':
            # CARTESIAN: re-align orientation after joint-space
    ...
    def next_state(self):
        transitions = {
            'PRE_GRASP': ('HOLD', 'PRE_GRASP_ALIGN'),
            'PRE_GRASP_ALIGN': 'LOWER',
            'LOWER': 'CLOSE_GRIP',
